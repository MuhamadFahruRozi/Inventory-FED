...
const Input_Penjualan = () => {
  ...
  const handleIdBarang = (e, index) => {
    ...
    const allHarga = [...harga]
    if (match && match.length > 0) {
      allNama[index] = match[0].nama_barang;
      allHarga[index] = match[0].harga_beli;
    } else {
      allNama[index] = '';
      allHarga[index] = '';
    }
  ...
  const handleJumlah = (e, index) => {
    ...
    totalPerItem[index] = allJumlah[index]*harga[index];
  ...
  const handleSubmit = (e) => {
    ...
    for (let index = 0; index < idBarang.length; index++) {
      ...
      !== undefined && jumlah[index] !== undefined && total[index] !== undefined) {
        console.log({notrajul: noTransaksi, idbar: idBarang[index], nambar: namaBarang[index]
          , jumjul: jumlah[index], tojul: total[index] })
      } else {
        console.log(`All data for row ${index+1} is not filled, sales is ${index+1} not recorded!`)
      }

      // console.log(inputData)
      // let formData = new FormData();
      // formData.append('notrajul', noTransaksi);
      // formData.append('idbar', idBarang[index]);
      // formData.append('nambar', namaBarang[index])
      // formData.append('jumjul', jumlah)
      // formData.append('tojul', total);
    ...
    // totalFormData.append('totalAll', totalAll);

    // const urlTotal ='http://localhost:8000/api/total/';
        ...
        <Form onSubmit={handleSubmit} >
          {
            ...
            <>
              <Row className='data-record' key={index}>
                ...
                <Col>
                  <input type="text" className='input' value={harga[index]} required readOnly />
                </Col>
                <Col>
